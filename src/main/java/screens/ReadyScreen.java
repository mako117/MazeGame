...
public class ReadyScreen extends ScreenAdapter {
    ...
    /**
     * ReadyScreen constructor that passes in the current MazeGame object.
    ...
    public void missionMenu() {
        ...
        game.batch.begin();
        game.batch.draw(game.backgroundTexture, game.camera.position.x - (Gdx.graphics.getWidth()) / 2,
                game.camera.position.y - (Gdx.graphics.getHeight()) / 2, Gdx.graphics.getWidth(),
        ...
        game.font.draw(game.batch, "The Dinosaurs are trying to stop their demise.\n" +
                ...
                game.camera.position.x - (Gdx.graphics.getWidth()) / 2 + 300, game.camera.position.y + 90);
        game.batch.end();
        missionStage.act();
        missionStage.draw();
    }
     ...
     * @param TILE_SIZE the size of the block tile
     * @param gameboard the actual board
     * @param nextScreen The next screen to be loaded
     */
    public void fullScreen(float delta, float time, int TILE_SIZE, Board gameboard, GameScreen nextScreen) {
        ...
        game.batch.begin();
        game.batch.draw(game.backgroundTexture, 0, 0, (Gdx.graphics.getWidth() * 1.8f),
    ...
    private void readyText() {
        ...
        game.font.draw(game.batch, String.format("READY!"), game.camera.position.x + viewport.getScreenWidth() / 10,
                game.camera.position.y + viewport.getScreenHeight() / 2);
        game.font.draw(game.batch, String.format("Disarm all the bombs.\n\n" +
                ...
                "Reach the end."), game.camera.position.x + viewport.getScreenWidth() / 10,
                game.camera.position.y + viewport.getScreenHeight() / 2 - 120);
        game.font.getData().setScale(1, 1);
    }
